...
In this example, we apply the BSFA-DGP model to a simulated data, which was generated under the scenario where latent factors were correlated and had small variability (scenario CS) in the paper. We show how to use different functions within the package to conduct a full data analysis.

The simulated data has already been saved in the package, named 'sim_fcs_truth.rda'. Before using functions within this package for analysis, the user has to prepare the initial values for latent factor scores $y$, regression coefficients of factor loadings $a$, binary variables of factor loadings $z$ and also the variance for residuals $\phi^2$. One possible way to obtain initials is to use [BFRM software](https://www2.stat.duke.edu/~mw/mwsoftware/BFRM/examples.html) to conduct a simple Bayesian Sparse Factor Analysis for independent data. We have already saved initials for use in the file named 'sim_fcs_init.rda'. Note that to save the time for knitting rmarkdown, we have also saved other results of this simulated data in the data file named 'sim_fcs_results_regular_8.rda', and users should be able to reproduce all results following the instructions in this vignette. 

First, we use the function 'mcem_parameters_setup' to generate R objects storing parameters in the model that are needed in the following Monte Carlo Expectation Maximization (MCEM) algorithm. We choose the model with the intercept term denoting subject-gene mean. 
...
# apply the function 'mcem_parameter_setup'
...
Next, these generated R objects, along with other model setups by the user, can be input to the function 'mcem_algorithm' for the Maximum Likelihood Estimate (MLE) of Dependent Gaussian Process (DGP) parameters.
...
The updating process of cross-correlations can be visualized using the function 'mcem_cov_plot'. The true cross-correlation was also displayed at last for comparison.
...
After initialization, multiple chains can be run in parallel (or iteratively as follows when the computing speed for one chain is fast), and posterior samples will be saved to the specified location.
...
# create a constant list storing constants
...
for (chain_index in 1:tot_chain){
  ...
  save(gibbs_after_mcem_load_chains_result,
       file = paste0("/path/to/folder/chain_", chain_index,"_result.RData"))
}
...
When it comes to the posterior analysis, we first focus on the continuous variables that do not need to alignment (due to the identifiability issue), including variables pred_x and phi (also including subject-gene mean and variance_g if the chosen model includes the intercept term). The function 'numerics_summary_do_not_need_alignment' not only assesses the convergence of these variables, but also return the posterior result of pred_x: 
...
With regard to the prediction performance, an overview can be provided through mean absolute error, mean width of the intervals, and the proportion of coverage:
...
Point estimate, and predictive interval for a specific gene of interest can also be retrieved. Taking the 2nd gene of the 1st person at the 1st new time point as an example, 
...
To check convergence situation:
...
and also visualize the trajectory of a specific factor for one specific person:
...
An overview of model's performance in estimating true factor scores can be calculated as:
